...
def learnGradientDecent(L: List[int], data:List[List[int]]):
    ...
    with tf.Session().as_default() as sess:
        ...
        for i in range(10000):
            vals = sess.run({'ll': loss, 'step': step, 'su':merged}, feed_dict={obs: data})
            print('{} - log-likelihood {:.3f}'.format(i, vals['ll']))
            trans = A.eval()
            init = pi.eval()
            mm = markov.Markov(trans, L, init)
            callback(m, vals['ll'], i, train_writer)


m = ipv4(5, 0.5, 0.2)
m.draw()
traces = [markov.genNondeterministicTrace(m, 9) for _ in range(0, 10000)]
print("original loglikelihood: " + str(markov.logLikelihood(m, traces)))

learnGradientDecent([0,1,2,3], traces)